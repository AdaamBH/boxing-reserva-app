...
## [2026-08-11] Diagnóstico en vivo: "No se ha podido cargar tu perfil" persistente en producción para una cuenta real

**Contexto:** Tras desplegar el arreglo anterior, el cliente reportó el nuevo mensaje de error (en vez del cuelgue infinito) al iniciar sesión en `boxing-reserva-app.vercel.app` con su cuenta real, insistiendo en que su conexión estaba bien. Se investigó a fondo antes de asumir que era de nuevo "la red":

1. **RLS/permisos descartados como causa** — se simuló exactamente la comprobación que hace PostgREST (`set local role authenticated; set local request.jwt.claims = '...sub=<user_id>...'; select ... from profiles`) directamente contra el proyecto de Supabase real: la fila se devuelve sin problema. La cuenta y sus permisos están bien.
2. **El propio `withTimeout` descartado como causa** — se ejecutó el mismo patrón (`supabase.from('profiles').select('*').eq('id', ...).single()` envuelto en `withTimeout`) desde Node contra el proyecto real: resuelve en <1s con la respuesta esperada. El mecanismo de timeout no tiene ningún bug que haga fallar la consulta por sí solo.
3. Con ambas causas de servidor descartadas, lo único que queda sin poder verificarse desde aquí es el tramo navegador↔red del cliente concreto en el momento del fallo — algo que no se puede depurar sin ver la consola/red de ese navegador.

**Decisión:** En vez de seguir adivinando, `ProtectedRoute` ahora muestra el **mensaje técnico real** (`profileError.message`) debajo del aviso amigable, para que la próxima vez dé una pista concreta (y sea capturable en una captura de pantalla) en vez de un genérico "comprueba tu conexión". Se añade un segundo botón, "Cerrar sesión y volver a entrar" (`supabase.auth.signOut()` + redirección a `/iniciar-sesion`), distinto de "Reintentar" (que solo recarga la página): si lo que está mal es una sesión guardada en este navegador (access token corrupto o caducado que ya no vale, no una red que fallaba puntualmente), **recargar repite el mismo error indefinidamente** porque `getSession()` sigue leyendo la misma sesión rota de `localStorage` — hace falta borrarla explícitamente, no solo reintentar la consulta.
