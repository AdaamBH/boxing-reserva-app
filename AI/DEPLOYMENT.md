...
- [ ] **Comprobado que el bundle desplegado apunta al proyecto de Supabase correcto**, no basta con mirar el panel de variables: descargar el JS publicado y buscar la URL compilada (`curl -s <sitio>/assets/index-*.js | grep -o 'https://[a-z0-9]*\.supabase\.co'`). La integración Vercel↔Supabase puede crear un proyecto vacío y reinyectar sus variables sin avisar — pasó, y dejó producción rota 28 días (ver `DECISIONS.md`, 2026-08-14). Si aparece un `PGRST205 / could not find the table`, esto es lo primero que hay que mirar.
- [ ] Dominio propio **verificado** en Resend (`resend.com/domains`) y `from` de Supabase cambiado a una dirección de ese dominio. Sin esto, Resend está en modo prueba y **solo entrega al email del titular de la cuenta** — cualquier registro con otra dirección falla con un 550 (ver `DECISIONS.md`, 2026-08-14). No basta con que el SMTP esté conectado.
- [ ] Supabase Auth configurado para usar Resend como SMTP personalizado (no el servicio de email de pruebas por defecto).
- [ ] **"Confirm email" reactivado** en Supabase → Authentication → Providers → Email, **si se llegó a desactivar**. La idea era desactivarlo en desarrollo para poder registrar cuentas sin dominio verificado (ver `DECISIONS.md`, 2026-08-14), pero **a fecha de 2026-08-14 seguía activo**: comprobado con un `POST /auth/v1/signup` real contra el proyecto, que devolvía `500 Error sending confirmation email` y no creaba ninguna fila en `auth.users`. Dejarlo desactivado en producción permite registrarse con un email ajeno sin verificar que sea tuyo, inaceptable con menores de por medio (`SECURITY.md`). El código de registro ya soporta ambos modos: es solo el ajuste.
- [ ] **Registro probado de verdad de punta a punta**, con un email que no sea el del titular de la cuenta de Resend. Es el único flujo crítico que a día de hoy nunca ha llegado a funcionar, y falla en el servidor (500), no en el código — un test verde en local no lo cubre.
